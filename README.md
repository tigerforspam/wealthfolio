...
### Running in Web Mode (via Docker)

Wealthfolio can also be run as a web application using Docker. This mode uses a Rust-based web server (`web-server` crate) to expose the backend API over HTTP, and the same frontend codebase is served.

**Prerequisites for Web Mode:**

- [Docker](https.www.docker.com/) installed and running.
- A running PostgreSQL database accessible to the Docker container.
...
1.  **Build the Docker Image:**
    ...
    ```
    This command builds a Docker image named `wealthfolio-web` which contains both the compiled Rust web server and the built frontend assets. The frontend is built with `VITE_RUN_ENV=WEB` to ensure it communicates with the HTTP API.

2.  **Run the Docker Container:**
    Once the image is built, run it as a container. You **must** provide the `DATABASE_URL` environment variable for the application to connect to your PostgreSQL database.
    ```bash
    docker run -p 4000:4000 -e DATABASE_URL="postgresql://YOUR_USER:YOUR_PASSWORD@YOUR_HOST:YOUR_PORT/YOUR_DATABASE_NAME" --name wealthfolio-web-container wealthfolio-web
    ```
    Replace the placeholder `postgresql://YOUR_USER:YOUR_PASSWORD@YOUR_HOST:YOUR_PORT/YOUR_DATABASE_NAME` with your actual database connection string.
    - `-p 4000:4000`: Maps port 4000 on your host to port 4000 in the container (where the web server listens).
    - `-e DATABASE_URL=...`: Sets the required database connection string.
    - `--name wealthfolio-web-container`: Assigns a name to your container for easier management.
...
3.  **Access the Web Application:**
    ...
    http://localhost:4000
    ```
    You should see the Wealthfolio application running in web mode. The API requests will be routed to `http://localhost:4000/rspc`.

**Notes for Web Mode:**
- The `web-server` component handles API requests.
- The Docker build process ensures that the `src/bindings.ts` file (TypeScript bindings for the API) is generated by `rspc` and used by the frontend build.
- Functionality that relies on desktop-specific capabilities (e.g., direct file system access for backups in the same way as Tauri, system tray integration) will not be available or will behave differently in the web version.
